...
void setup(const char *base_uri, const char *screen, int inspect, int debug,
    ...
    //
    // This used to qputenv() our own flags unconditionally, silently
    // discarding any value already set externally (e.g. via a systemd
    // unit's own Environment= line). Now appends to whatever is already
    // present instead, so an externally-set value and our own required
    // flags can coexist.
    ...
    // text renders visibly larger under QtWebEngine than under other
    // Xibo players (CEF/Windows client, the Go player) on the same
    // physical screen, both in arexibo's own "resource" widgets (e.g.
    // the clock-digital widget) and in `webpage render="native"`
    // widgets showing real external sites. DPI/devicePixelRatio/the
    // widget-box-overflow issue, and separately QT_FONT_DPI (changes
    // what QScreen reports but has zero effect on QtWebEngine's own
    // actual rendered CSS font-size), were investigated and ruled out
    ...
    //
    // Controlled by the AREXIBO_FONT_SCALE environment variable, to
    // allow experimenting with the right factor on real hardware
    // without needing a rebuild each time -- unset or empty: no script
    // is injected at all, today's default behavior. Set to a number
    // (e.g. "0.95"): every element's own computed font size, in every
    // frame of every WebEngine view (main layout page, overlay,
    // "native" webpage widgets, and -- since this uses the profile-
    // level script mechanism with runsOnSubFrames enabled -- any
    // iframe within them regardless of origin, including "resource"
    // widgets that fetch their HTML directly from the CMS) is scaled
    // by that factor. A value outside (0, 2] is rejected (logged, no
    // injection) as almost certainly a typo rather than an intentional
    // extreme request.
    //
    // Deliberately NOT scaling anything else (images, layout boxes,
    // etc.) -- only `font-size`, since specifically (and only) text
    // appeared larger, not other content.
    ...
    // bundle.min.js) handles this by comparing the widget's declared
    // "original" design size against `$(window).width()/.height()` --
    // but since arexibo renders each such widget in an `<iframe>` sized
    // to exactly its XLF region dimensions, that comparison is always
    // 1:1 by construction, so xiboLayoutScaler always computes a no-op
    // scale factor of 1 and never shrinks anything, regardless of
    // whether the content itself (fixed font-sizes authored against
    // some other assumed canvas) actually fits.
    //
    // Architecture note (why this lives here, at the profile level, and
    // not as a parent-page function): widgets are served from one of
    // several loopback origins (see server::HTML_SHARD_COUNT, working
    // around Chromium's per-origin connection limit) -- meaning a
    // widget's own iframe can be genuinely cross-origin relative to the
    // page that embeds it, and the browser's same-origin policy would
    // block the parent from reaching into `iframe.contentDocument`
    // directly. Injecting a self-contained script into every frame
    // (main layout, overlay, and every widget iframe alike, via
    // runsOnSubFrames below) sidesteps this: each frame reads its own
    // intended box size from its own URL's query string
    // (`arexiboShrinkW`/`arexiboShrinkH`, see layout.rs's `write_media`)
    // and shrinks its own `document.body` if needed -- no cross-frame
    // reaching-in required. The distinct query param names (not just
    // `w`/`h`) matter because this script also runs in native
    // `webpage render="native"` widgets showing real external sites --
    // a name unlikely to collide with any such site's own query
    // parameters avoids ever acting on them by mistake.
    //
    // Deliberately never scales up (only ever shrinks, scale <= 1) --
    // content that already fits is left exactly as authored. Re-checked
    // on every DOM mutation (via MutationObserver, debounced) rather
    // than only once at load, since some widgets populate their real
    // content asynchronously after the page's `load` event (the
    // clock-digital template renders its own placeholder spans empty
    // at first, filling in the actual time/date text slightly later).
        ...
        (function() {
            ...
            var onRenderPatchInterval = setInterval(function() {
                for (var key in window) {
                    if (patchedOnRender[key]) continue;
            ...
            function tryShrink() {
                ...
                if (scale < 1) {
                    // Diagnostic log, caught via LoggingPage -- useful
                    // for widget content whose natural size assumptions
                    // don't match its own region's declared box (e.g. a
                    // full-canvas "Elements" widget). Gated behind
                    // --web-debug, since this would otherwise add
                    // permanent noise to every normal run.
                    if (window.arexiboDebug) {
                        console.log('arexibo-shrink: target=' + w + 'x' + h +
                                    ' measured=' + sw + 'x' + sh + ' scale=' + scale);
                    }
                ...
                // A worldclock-digital-date widget renders as a small
                // dark box in the corner of its own much larger region,
                // rather than filling it -- its handlebars template
                // hardcodes data-width="200" data-height="80" as its
                // native design size, regardless of the actual region
                // size. Same root cause as the shrink case above
                // (xiboLayoutScaler comparing container size against
                // globalOptions.originalWidth/Height, which arexibo
                // always sets equal to the container by construction,
                // so the ratio it computes is always exactly 1 and it
                // never does anything) -- just the opposite direction:
                // undersized content that needs to grow to fill its
                // box. A generous but bounded cap (MAX_GROW_SCALE)
                // avoids grotesquely scaling up content that's
                // tiny/broken for an unrelated reason.
    ...
    // -- previously all unconditional, adding permanent noise to every
    // run. `window.arexiboDebug` is injected into every frame (main,
    // overlay, and every widget iframe alike) so the same flag reaches
    // JS generated at very different points without duplicating it.
