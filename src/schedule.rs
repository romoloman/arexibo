...
struct ScheduleEntry {
    ...
    #[serde(default)]
    duration: Option<i64>,
    // Sync Group / Synchronised Event marker (see syncgroup.rs) --
    // confirmed real from a live CMS schedule.xml, generated by a real
    // "Add Synchronised Event" schedule entry: `syncEvent="1"`. No
    // `syncGroupId` accompanies it on the `<layout>` element itself --
    // the display's own Sync Group role (Lead/Follower/none) is already
    // known from RegisterDisplay's own `<syncGroup>` field (see
    // config.rs's own SyncRole), so the schedule doesn't need to repeat
    // it. This flag alone marks the entry as sync-gated: even once
    // `layouts_now()` would otherwise consider it the current layout,
    // it must not be shown immediately -- see mainloop.rs's own
    // handling, which instead waits for the Lead's own "switch now"
    // signal (SyncGroup::Command) before actually applying it. Absent
...
impl Schedule {
    pub fn parse(tree: &Element) -> Result<Self> {
        ...
        for layout in tree.find_all("layout") {
            ...
            // "1" confirmed real (see ScheduleEntry's own doc comment);
            // defaults to false/absent for every ordinary entry, and
            // tolerates any other non-"1" value the same way (also
    ...
    /// Whether `layoutid` (assumed to be one currently returned by
    /// `layouts_now()`) is *sync-gated* right now -- i.e. has a
    /// currently-active schedule entry (same from/to/criteria check as
    /// `layouts_now()`'s own first filtering step) with
    /// `syncEvent="1"` (see ScheduleEntry's own doc comment). Kept
    /// deliberately independent of the interrupt/share-of-voice
    /// resolution above -- there's no real-world data yet on how an
    /// Interrupt Layout and a Synchronised Event would combine, so
    /// this only answers "does *this specific* layout id have an
    /// active sync-gated entry", not anything about priority
    /// resolution (already handled by `layouts_now()` itself, which
    /// this is meant to be checked alongside, not instead of).
    pub fn is_sync_gated(&self, layoutid: LayoutId, criteria: &CriteriaStore) -> bool {
        let now = OffsetDateTime::now_local().unwrap();
        self.schedules.iter().any(|e| {
            e.layoutid == layoutid && e.sync_event
                && e.from <= now && now <= e.to && self.criteria_satisfied(e, criteria)
        })
    }

    /// Whichever layout id is *this display's own* currently active
    /// sync-gated candidate right now, if any -- unlike `is_sync_gated`
    /// (which checks a specific, already-known id), this answers "what
    /// is it" without needing one in advance. Needed for Sync Group's
    /// own sync_keys matching (see mainloop.rs's own
    /// resolve_layout_for_sync_keys): each display (Follower or Lead)
    /// resolves a received sync_keys set against *its own* schedule,
    /// which may name a genuinely different layout id than the Lead's
    /// own (Wall Sync: each display shows a different layout of a
    /// shared composition) -- this is how a display finds its own
    /// candidate to check. Same highest_priority resolution as
    /// `layouts_now()` uses, restricted to sync_event entries; no
    /// real-world data yet on how this would combine with an Interrupt
    /// Layout (share_of_voice), so deliberately not attempted here.
    pub fn active_sync_gated_layout(&self, criteria: &CriteriaStore) -> Option<LayoutId> {
        let now = OffsetDateTime::now_local().unwrap();
        let active = self.schedules.iter()
            .filter(|e| e.sync_event && e.from <= now && now <= e.to
                        && self.criteria_satisfied(e, criteria));
        Self::highest_priority(active).first().map(|e| e.layoutid)
    }
...
mod overlay_tests {
  ...
  <default file="614" duration="60"/>
    ...
    #[test]
    fn parses_real_sync_group_mirror_event_xml_from_user() {
        // Exact structure from a real schedule.xml the user shared,
        // captured from the Lead display of a real Sync Group (Mirror
        // Sync) after scheduling a real "Add Synchronised Event" on the
        // CMS -- `syncEvent="1"`, no `syncGroupId` on the <layout>
        // element itself (see ScheduleEntry's own doc comment for why).
        let xml = r#"<schedule generated="2026-08-28 16:24:27" filterFrom="2026-08-28 16:00:00" filterTo="2026-08-30 16:00:00">
  <layout file="1014" fromdt="1970-01-01 01:00:00" todt="2038-01-19 04:14:07" scheduleid="264" priority="0" syncEvent="1" shareOfVoice="0" duration="60" isGeoAware="0" geoLocation="" cyclePlayback="0" groupKey="0" playCount="0" maxPlaysPerHour="0"/>
  <default file="854" duration="10"/>
</schedule>"#;
        let tree = Element::from_reader(xml.as_bytes()).unwrap();
        let sched = Schedule::parse(&tree).unwrap();
        assert_eq!(sched.schedules.len(), 1);
        assert!(sched.schedules[0].sync_event,
                "a real syncEvent=\"1\" entry must be parsed as sync-gated");
        assert_eq!(sched.schedules[0].layoutid, 1014);
    }

    #[test]
    fn is_sync_gated_confirms_a_real_sync_event_layout() {
        let xml = r#"<schedule generated="2026-08-28 16:24:27" filterFrom="2026-08-28 16:00:00" filterTo="2026-08-30 16:00:00">
  <layout file="1014" fromdt="1970-01-01 01:00:00" todt="2038-01-19 04:14:07" scheduleid="264" priority="0" syncEvent="1" shareOfVoice="0" duration="60" isGeoAware="0" geoLocation="" cyclePlayback="0" groupKey="0" playCount="0" maxPlaysPerHour="0"/>
  <default file="854" duration="10"/>
</schedule>"#;
        ...
        assert!(!sched.is_sync_gated(854, &criteria));
    }

    #[test]
    fn is_sync_gated_is_false_for_an_ordinary_non_synchronised_layout() {
        let xml = r#"<schedule generated="2026-08-06 11:39:20" filterFrom="2026-08-06 11:00:00" filterTo="2026-08-08 11:00:00">
  <layout file="614" fromdt="1970-01-01 01:00:00" todt="2038-01-19 04:14:07" scheduleid="210" priority="0" syncEvent="0" shareOfVoice="0" duration="60" isGeoAware="0" geoLocation="" cyclePlayback="0" groupKey="106" playCount="0" maxPlaysPerHour="0"/>
  <default file="614" duration="60"/>
</schedule>"#;
