...
impl Cache {
    ...
    /// expected name, regardless of version -- unlike `has()` above
    /// (which requires an md5 match against the latest version). Used
    /// by `is_exempt_as_currently_playing_layout` in mainloop.rs.
